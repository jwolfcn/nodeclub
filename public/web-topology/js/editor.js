...
propertyPanel.prototype.saveToplogy = function (showAlert) {
        ...
        // }
        $.ajax({
            url: '/zxa/' + "topologyManage/saveTopologyJSON",
            async: true,
            type: "POST",
            dataType: "json",
            data: {
...
const loadFromServer = function ({backImg,templateId,topologyId,topoLevel},fail) {
    $.ajax({
        url: '/zxa/topologyManage/getTopologyJSON',
        async: false,
        type: "GET",
        dataType: "html",
        data: {
        ...
        success: function (response) {
            ...
            if (err && err != "ok") {
                ...
                var topologyJson =eval('(' + response.topologyJson + ')');
...
const loadFromStatic = function ({backImg,templateId,topologyId,topoLevel}) {
    $.ajax({
        url: './topology.html',
        async: false,
        type: "GET",
...
propertyPanel.prototype.loadTopology = function (backImg,templateId,topologyId,topoLevel) {
    ...
    // loadFromStatic({backImg, templateId,topologyId,topoLevel})
    loadFromServer({backImg, templateId,topologyId,topoLevel}, function(){loadFromStatic({backImg, templateId,topologyId,topoLevel})})
    };
...
networkTopologyEditor.prototype.init = function (backImg,templateId,topologyId,stageJson,templateName) {
    ...
    this.scene.dbclick(function(e){
        ...
        if(e.target != null && e.target instanceof JTopo.Node){
            ...
            //双击设置隐藏域的值
            $('#chartId').val(editor.currDeviceId)
            $("#nodeModuleIdHidden").val(e.target.deviceId);
            $("#nodeEnvTemplateIdHidden").val(editor.templateId);
            getDeviceIdByChartKey&&getDeviceIdByChartKey(e.target.deviceId);
        }
    });
    //数去焦点,设置节点文字
